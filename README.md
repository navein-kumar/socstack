...
| TheHive | MISP | MISP API key | Threat intelligence sync |
...
| **B** | TheHive → MISP server integration | 2 min |
| **C** | Cortex → Enable VirusTotal analyzer + other analyzers | 5 min |
| **D** | MISP → Change default admin password | 1 min |
| **E** | Wazuh Dashboard → SSO role mapping (admin + read-only) | 3 min |
| **F** | n8n → Import workflow, configure Redis/SMTP/TheHive, connect Wazuh webhook | 10 min |
| **G** | Grafana → Verify auto-provisioned datasource, create dashboards | 5 min |
...
|-----|----------|-------------|----------|
| `MISP_API_KEY` | `.env.deployed` | post-deploy.py (from MISP DB) | TheHive → MISP integration |
